...
void get()
{
    printf("ENTER YOUR FIRST NAME:  ");                           //gettign the name//
    scanf("%s",&n1.name);
    n1.score=0;
    n1.money=0;
...
void game()
{
    ...
    char options[][100]={"1.Guido van Rossum","2.Dennis Ritchie","3.Anders Hejlsberg","4.James Gosling",
     ...
     int answers[5]={2,3,1,4,1};
     int numberofquestions= sizeof(questions)/ sizeof(questions[0]);            //getting the size of an array//
...
void high_s()
{
    ...
    else if(n1.score==0&&n1.s_score==0){
        printf("\n\t\tYOU HAVEN't ANY HIGHEST SCORE !");
    }
